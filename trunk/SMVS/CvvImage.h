#pragma once

#include "cv.h"
#include "opencv2/highgui/highgui.hpp"
#include "opencv2/core/core.hpp"
#include "opencv2/imgproc/imgproc.hpp"
#include "opencv2/imgproc/imgproc_c.h"

using namespace cv;

class  CvvImage
{
public:
	CvvImage();
	~CvvImage();
	bool  Create(int width, int height, int bits_per_pixel, int image_origin = 0);
	bool  Load(const char* filename, int desired_color = 1);
	bool  LoadRect( const char* filename,int desired_color, CvRect r);

//#if defined WIN32 || defined _WIN32
//	bool  LoadRect(const char* filename,int desired_color, RECT r)
//	{
//		return LoadRect( filename, desired_color,cvRect( r.left, r.top, r.right - r.left, r.bottom - r.top ));
//	}
//#endif

	bool  Save(const char* filename);
	void  CopyOf(CvvImage& image, int desired_color = -1);
	void  CopyOf(IplImage* img, int desired_color = -1);
	void  CreateFromMat(cv::Mat* img, int desired_color = -1);

	IplImage* GetImage() { return m_img; };
	void  Destroy(void);


	int Width() { return !m_img ? 0 : !m_img->roi ? m_img->width : m_img->roi->width; };
	int Height() { return !m_img ? 0 : !m_img->roi ? m_img->height : m_img->roi->height;};
	int Bpp() { return m_img ? (m_img->depth & 255)*m_img->nChannels : 0; }

	void  Fill(int color);
	void  Show(const char* window);

//#if defined WIN32 || defined _WIN32
//	void  Show(HDC dc, int x, int y, int width, int height,int from_x = 0, int from_y = 0);
//	void  DrawToHDC(HDC hDCDst, RECT* pDstRect);
//#endif

private:
	IplImage*  m_img;
};

//typedef CvvImage CImage;
