...
CPCI2330Device::~CPCI2330Device(void)
{
	if(m_bIoCardReady)
	{
		...
		PCIE2330_ReleaseDeviceInt(m_hIoCard);	// 释放中断资源
		PCIE2330_ReleaseDevice(m_hIoCard);		// 释放设备对象
	}
...
bool CPCI2330Device::CreatePCIDevice()
{
	try
	{
		...
		m_hIoEvent = PCIE2330_CreateSystemEvent(); // 创建中断事件句柄
		...
		PCIE2330_PARA_INT IntPara; // 用于中断的信号源确定(只读)
