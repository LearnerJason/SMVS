...
void CModelToolDlg::OnBnClickedButtonSaveModel()
{
	...
	if(isCircle)
	{
		...
		{
			ret = create_shape_model(retImg, m_numLevels, HTuple(0).Rad(), HTuple(360).Rad(), "auto","auto", "use_polarity", "auto", "auto", &m_modelID);
		}
	}
	else
	{
...
LRESULT CModelToolDlg::OnRangeSliderChanged(WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		if(m_newImg.Id() != H_EMPTY_REGION )
		{
			...
			HTuple centerXY;
			CenterDefined(m_newImg, (float)0.8, centerXY);	
...
void CModelToolDlg::CreateImageWindow(CRect& rcScreen)
{
	...
	try
	{
		...
		gen_image1(&initIimage,"byte",width,height,(long)&image[0]);
